...
void main() {
  ...
  group('CounterView', () {
    testWidgets('renders current CounterCubit state', (tester) async {
      when(() => counterCubit.state).thenReturn(CounterState(countValue: 42));
    ...
    testWidgets('tapping on increment button invokes increment',
        (tester) async {
      when(() => counterCubit.state).thenReturn(CounterState(countValue: 0));
    ...
    testWidgets('tapping on decrement button invokes decrement',
            (tester) async {
          when(() => counterCubit.state).thenReturn(CounterState(countValue: 0));
